...
<div class="card-container">
    ...
    if ($queryResults > 0) {
        while ($row = mysqli_fetch_assoc($results)) {
            ...
            // Include item_id in the HTML for the delete button
            echo "</div>
                        <button class='prev-button'>Previous</button>
                        <button class='next-button'>Next</button>
                        <button class='card__button card__message-button'>Message Seller</button>
                        <button class='card__button card__delete-button' data-item-id='" . $productId . "'>Delete Item</button>
                    </div>
                </div>";
        }
    } else {
...
<script>
    ...
    function openMessageForm(owner, itemName) {
        // Check if the event originated from the delete button
        if (!owner && !itemName) {
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        var messageButtons = document.querySelectorAll('.card__message-button');
        var deleteButtons = document.querySelectorAll('.card__delete-button');
